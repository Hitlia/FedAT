    ...
    def test(self):
        if self.model_name == 'anomaly_transformer':
            self.model.load_state_dict(
                torch.load(
                    os.path.join(str(self.model_save_path), str(self.dataset) + '_checkpoint_05.pth')))
            self.model.eval()
            temperature = 50
            ...
            # thresh = pot_eval(train_energy, test_energy, q=1e-3, level=0.02)
            ...
            errors = Errors(test_energy)
            errors.process_batches(test_energy)

            pred = np.zeros_like(test_labels)
            for e_seq in errors.E_seq:
                pred[e_seq[0]:e_seq[1]]=1
            ...
            # self.model.eval()
            # if os.path.isfile(os.path.join(str(self.model_save_path), str(self.model_name) + '_checkpoint.pkl')) == False:
            data_train = []          
            for i, (input_data, labels) in enumerate(self.train_loader):
            ...
            print("Training Done.\n")
            #     # self.model.save_weights(os.path.join(str(self.model_save_path), str(self.model_name) + '_checkpoint.h5'))
            #     with open(os.path.join(str(self.model_save_path), str(self.model_name) + '_checkpoint.pkl'), 'wb') as f:
            #         dill.dump(self.model, f)
            # else:
            #     # self.model.load_weights(os.path.join(str(self.model_save_path), str(self.model_name) + '_checkpoint.h5'))
            #     with open(os.path.join(str(self.model_save_path), str(self.model_name) + '_checkpoint.pkl'), 'rb') as f:
        ...
        pred = np.array(pred)
        gt = np.array(gt)
        # pred_for_clip = np.zeros_like(len(label_slice))
        # for clip_idx in range(len(pred)//self.win_size):
        #     pred_clip = pred[clip_idx*self.win_size:clip_idx*self.win_size+self.win_size]
        #     if np.sum(pred_clip)>self.win_size/2:
        #         pred_for_clip[clip_idx] = 1
        #     else:
        #         pred_for_clip[clip_idx] = 0
        # pred_for_clip = np.array[pred_for_clip]
        ...
        # accuracy_clip = accuracy_score(label_slice, pred_for_clip)
        ...
        txtDir = txtDir_base + self.model_name + '_test_results.txt'
