...
class FourierBlock(nn.Module):
    def __init__(self, window_size,in_channels, out_channels, seq_len, modes=0, mask_flag=True, attention_dropout=0.1,mode_select_method='random',output_attention=False):
    ...
    def forward(self, q, k, v, sigma, attn_mask):
        # size = [B, L, H, E]
        B, L, H, E = q.shape
        _, S, _, D = v.shape
        ...
        prior = 1.0 / (sqrt(2 * math.pi) * sigma) * torch.exp(-prior ** 2 / 2 / (sigma ** 2))
