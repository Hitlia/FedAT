...
from model.new_transformer import AnomalyTransformer
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        torch.save(model.state_dict(), os.path.join(path, str(self.dataset) + '_checkpoint_test.pth'))
...
class Solver(object):
    ...
    def build_model(self):
        ...
        if self.model_name == 'anomaly_transformer':
            self.model = AnomalyTransformer(win_size=self.win_size, num_points=self.win_size, enc_in=self.input_c, c_out=self.output_c, num_points_out=self.win_size, e_layers=3)
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            for u in range(len(prior)):
                series_loss += (torch.mean(my_kl_loss(series[u], prior[u].detach())) + torch.mean(my_kl_loss(prior[u].detach(),series[u])))
                prior_loss += (torch.mean(my_kl_loss(prior[u],series[u].detach())) + torch.mean(my_kl_loss(series[u].detach(),prior[u])))
            series_loss = series_loss / len(prior)
            prior_loss = prior_loss / len(prior)

            rec_loss = self.criterion(output, input)
            loss_1.append((rec_loss + self.k * (prior_loss - series_loss)).item())
            ...
            for i, (input_data, labels) in enumerate(self.train_loader):
                ...
                for u in range(len(prior)):
                    series_loss += (torch.mean(my_kl_loss(series[u], (
                            prior[u]).detach())) + torch.mean(
                        my_kl_loss((prior[u]).detach(),
                                   series[u])))
                ...
                loss1_list.append((rec_loss + self.k * (prior_loss+series_loss)).item())
    ...
    def test(self):
        if self.model_name == 'anomaly_transformer':
            self.model.load_state_dict(
                torch.load(
                    os.path.join(str(self.model_save_path), str(self.dataset) + '_checkpoint_add.pth')))
            self.model.eval()
            temperature = 50
            ...
            thresh = pot_eval(train_energy, test_energy, q=1e-4, level=0.999)
