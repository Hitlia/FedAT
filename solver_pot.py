...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, val_loss2, model, path):
        ...
        torch.save(model.state_dict(), path) #os.path.join(path, str(self.dataset) + '_checkpoint_freq.pth'))
...
class Solver(object):
    ...
    def test(self):
        if self.model_name == 'anomaly_transformer_freq' or self.model_name == 'anomaly_transformer':
            self.model.load_state_dict(
                torch.load(self.model_save_path))
                    #os.path.join(str(self.model_save_path), str(self.dataset) + '_checkpoint_freq.pth')))
            self.model.eval()
            temperature = 50
            ...
            thresh = pot_eval(train_energy, test_energy, q=1e-3, level=0.999)
        ...
        txtDir = self.results_save_path #txtDir_base + self.model_name + '_test_results_pot_freq_0924.txt'
